...
>
> **용어:** 소스 변수명 `userId`는 내부 식별자로 유지하되, **사용자에게 보이는 모든 문자열은 "복구 코드"**로 표기한다(로그인 없는 재진입 키, §UX 원칙2). "재진입"은 "코드로 들어가기"로 표기한다.
>
...
### 8.1 세션 진입 (가입 / 코드로 들어가기) — `SignUpScreen`
로그인 화면(png) + 회원가입 화면 비주얼. 백엔드는 로그인 엔드포인트 없음 → **가입(`POST /auth/signup`)** 과 **복구 코드로 재진입**(이미 발급받은 코드 입력)으로 구성. **640dp 좁은 컨테이너 중앙**. 이름 필드·소셜·약관체크 제외("AI 결과 책임" 캡션은 유지).
...
│                              │
│  [탭] 가입하기 | 코드로 들어가기 │  ← Segmented (가입 / 복구 코드)
│                              │
│  이메일                       │  label
│  ┌──────────────────────┐    │  RmTextField (email)
│  비밀번호                     │
...
│                              │
│  [  가입하고 시작하기  ]       │  PrimaryButton (56dp)
│                              │
│ 이미 복구 코드가 있으신가요? 코드로 들어가기 │  TextLink
└──────────────────────────────┘
```
- 컴포넌트: BrandTopBar, RmTextField×2, PrimaryButton, TextLink, Segmented.
- "코드로 들어가기" 탭: **복구 코드**(라벨 "복구 코드", placeholder "가입할 때 받은 복구 코드", UUID 표기 제거) 입력 → 검증 후 보관 → 홈. (로그인 대체)
- 에러: 이메일 형식/비번 8자 미만 → InlineError. 가입 실패(중복 등) → Snackbar(서버 한국어 메시지).
- 성공: 가입 시 **복구 코드 보관 고지 화면**("내 복구 코드" + 복사 + 경고) → 확인 후 홈.
...
마이페이지 레퍼런스에서 **백엔드 범위만**: 이메일 표시, 복구 코드 표시·복사, 로그아웃(세션 클리어), 회원 탈퇴(`DELETE /me`). 아바타/성명/학력/경력/자격/수상 섹션은 제외. **헤더 계정 메뉴에서 진입**(바텀내비 의존 제거). 640dp 좁은 컨테이너.
...
│ └ 복구 코드  ●●●●(복사)     ┘ │   (재진입용 복구 코드 복사 가능)
│ ┌ 복구 코드는 다시 들어올 때 열쇠예요 ┐ │ InfoCard (보관 고지)
│                              │
│ 로그아웃                  ›   │ textBody (세션 클리어)
│ 회원 탈퇴                 ›   │ danger 텍스트 → ConfirmDialog
└──────────────────────────────┘
```
- 사용자 노출 문자열은 모두 "복구 코드"(변수 `userId`는 유지). 복사 a11y `contentDescription="복구 코드 복사"`.
...
- [ ] 사용자 노출 문자열이 "복구 코드"·"코드로 들어가기"(변수명 userId는 유지), a11y 포함.
