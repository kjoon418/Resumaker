...
│                                              │
│ ▸ 이력서·포트폴리오                            │ headingM
│ ┌ InfoCard "이력서·포트폴리오 만들기" [만들기 시작]┐│ InfoCard (넓은 배너)
└──────────────────────────────────────────────┘
```
...
- 산출물 영역: 넓은 배너 InfoCard "이력서·포트폴리오 만들기" → 생성 화면(§8.7) 진입. 빈 자료면 예방 카피(기록 유도, ComingSoon §5.11).
...
### 8.7 산출물 생성·열람 — `ArtifactCreateScreen` / `ArtifactScreen`
백엔드 생성 파이프라인 연동 완료(이력서/포트폴리오 1차 생성·열람). 생성은 LLM 호출이라 수십 초 걸릴 수 있으니 로딩 UX 필수. **부분 실패는 정직하게 고지**(가짜 성공 금지 — §5.11 정직성 원칙 계승).

**생성 — `ArtifactCreateScreen`** (640dp 좁은 컨테이너)
```
┌── 전체폭 PageHeader (back "이력서·포트폴리오 만들기") ──┐
├────────────[ 640dp 컨테이너, 중앙 ]────────────┤
│ 종류  ( • 이력서 ) ( 포트폴리오 )  │ 단일 선택 행(점 표시=선택)
│ 경험 *  ☑여러 개 선택              │ 다중 선택 행(점 표시), 1개+ 필수
│   • 경험 카드 …                   │
│ 목표 *  ( • 토스 백엔드 )          │ 단일 선택 행, 필수
│ 양식 *  ( • 신입개발자표준 )       │ 이력서일 때만, 필수 (포트폴리오는 숨김)
│                              │
│  [   만들기   ]               │ PrimaryButton, 필수 미충족 시 비활성(게이트)
│  생성에는 시간이 걸릴 수 있어요    │ caption, 생성 중 버튼 로딩
└──────────────────────────────┘
```
- 빈 경험(자료 없음)이면 ComingSoon(§5.11) 예방 카피로 분기 → 기록 유도(도메인 흐름 1·2).
- 필수: 이력서=경험+목표+양식, 포트폴리오=경험+목표. 미충족 시 제출 비활성. 생성 중 중복 제출 차단.
- 서버 409(빈 묶음, action=ADD_EXPERIENCE)는 ErrorBanner로 표면화 + 경험 추가 유도.

**열람 — `ArtifactScreen`** (생성 결과·`GET /artifacts/{id}`)
```
┌── 전체폭 PageHeader (back, 종류 라벨) ──┐
├────────────[ 컨테이너 ]────────────┤
│ [ 전체 복사 ]                      │ 성공 항목만 복사(실패 항목 제외)
│ ┌ 항목: 한줄소개  [완료]      📋 ┐ │ StatusBadge + 항목 복사
│ │  …생성된 내용…                │ │
│ │  출처 경험 2건                 │ │ caption
│ ├ 항목: 경력  [검증 실패]        ┐ │ 실패 배지(가짜 내용 없음)
│ │  이 항목은 만들지 못했어요.     │ │ 실패 사유만 고지, 내용 비움
│ └──────────────────────────┘ │
└──────────────────────────────┘
```
- 상태 배지: 완료(GENERATED)/생성 실패(GENERATION_FAILED)/검증 실패(VALIDATION_FAILED).
- **정직성**: 실패 항목은 가짜 내용을 만들지 않고 사유만 고지, 복사에서 제외(도메인 §306·신뢰성 가드레일).
- 복사는 클라이언트 책임(§6) — 항목별·전체 클립보드 복사.
- 딥링크 `/artifacts/{id}` 새로고침 시 GET으로 재조회.
...
- [ ] 산출물 생성·열람이 백엔드 연동(가짜 동작 0건); 부분 실패 항목은 사유만 고지하고 복사에서 제외. 빈 자료일 때만 ComingSoon 예방 분기.
