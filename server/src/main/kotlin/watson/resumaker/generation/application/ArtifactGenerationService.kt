...
class ArtifactGenerationService(
    ...
    fun generateResume(ownerId: UserId, command: GenerateResumeCommand): GenerationResponse {
        ...
        // 2·3. (tx 밖) 외부 생성 + 자동 검증(Cycle C seam)
        val output = generationPort.generate(material)
        groundingValidator.validate(output.sections)
        ...
        return requireNotNull(
            transactionTemplate.execute {
                ...
                persistAndMap(ownerId, ArtifactKind.RESUME, templateSnapshot, output, material)
            },
    ...
    fun generatePortfolio(ownerId: UserId, command: GeneratePortfolioCommand): GenerationResponse {
        ...
        return requireNotNull(
            transactionTemplate.execute {
                persistAndMap(ownerId, ArtifactKind.PORTFOLIO, templateSnapshot = null, output = output, material = material)
            },
    ...
    private fun persistAndMap(
        ...
        val reconciled = reconcileSections(kind, material, output.sections)
    ...
    private fun reconcileSections(
        ...
        val kindCompatible = sections.filter { it.sectionKind in allowedKinds }
        return when (kind) {
            ArtifactKind.RESUME -> {
                // [MED-1] 양식 스냅샷 키 집합에 존재하는 항목만 보존.
                val snapshotKeys = material.templateSections.map { it.definitionKey }.toSet()
                kindCompatible.filter { it.definitionKey in snapshotKeys }
            }
            ArtifactKind.PORTFOLIO -> {
                ...
                kindCompatible.filter { section ->
                    section.definitionKey in selectedKeys && seen.add(section.definitionKey)
                }
            }
        }
    }
    ...
    private fun GeneratedSection.toArtifactSection(): ArtifactSection =
        ArtifactSection.create(
            definitionKey = definitionKey,
            sectionKind = sectionKind,
            content = SectionContent.of(content),
            status = if (succeeded) SectionStatus.GENERATED else SectionStatus.GENERATION_FAILED,
            sourceExperienceIds = sourceExperienceIds,
            factGroundings = factGroundings.map { grounding ->
