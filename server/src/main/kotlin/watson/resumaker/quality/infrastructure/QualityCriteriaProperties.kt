...
data class QualityCriteriaProperties(
    ...
    companion object {
         ...
         *
         * **어절 경계 앵커(AI-01 후속):** 사전 매칭([QualityCriteriaDictionary.containsAtWordStart])은 어절 시작
         * 경계를 강제하는데, 정규식은 raw `find`라 같은 규율이 없으면 "보수만큼/접수만/횟수만"(→ `수만`), "1024배열"
         * (→ `배`)처럼 단어 가운데서 오탐해 **유료 AUTO_REWRITE를 오발**한다(AI-06이 없애려던 거짓양성 부류).
         * - `수(십\|백\|천\|만)`: 앞에 글자/숫자가 없을 때만(lookbehind) — "수만 명"은 잡고 "보수만큼"은 거른다.
         * - `\d+\s*배`: 뒤에 한글/라틴 글자가 이어지지 않을 때만(lookahead) — "2배 향상"은 잡고 "1024배열/배송"은
         *   거른다. 부수로 "2배의"처럼 조사가 바로 붙은 표현은 빠지는 recall 손실이 있으나, 유료 처치의 거짓양성을
         *   막는 정밀도 우선이 안전하다(의도된 트레이드오프).
         */
        private val DEFAULT_VAGUE_METRIC_PATTERNS = listOf(
            """\d+\s*%\s*(증가|증대|개선|향상|단축|감소|절감|상승|성장)""",
            """\d+\s*배(?![가-힣A-Za-z])""",
            """(?<![가-힣A-Za-z0-9])수(십|백|천|만)""",
        )
